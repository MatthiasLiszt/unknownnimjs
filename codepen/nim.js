...
class Field extends React.Component {
  ...
  render() {
    ...
    switch(this.props.s)
     {  
      case '0':  
        return <button onClick={thC} className={f} id={id}  alt="_" />;

      case 'x':
        return <button className={xpng} id={id}  alt="x" />; 
      case 'o':
        return <button className={opng} id={id}  alt="o" />;
      case 'a':  
        return <button onClick={thC} className={f} id={id} alt="_" />; 
      default: 
        return <button className={errorI} id={id}  alt="error!" />; 
     }   
   } 
...
ReactDOM.render(<PlayerMode/>, document.getElementById('app')); // Program Entry
